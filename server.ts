...
function apiToDb(apiData: any) {
  ...
  if (apiData.buttonText !== undefined) result.buttonText = apiData.buttonText;
  if (apiData.buttonLink !== undefined) result.buttonLink = apiData.buttonLink;
  if (apiData.displayOrder !== undefined) result.displayOrder = apiData.displayOrder;
  if (apiData.isActive !== undefined) result.isActive = apiData.isActive;
  return result;
}
...
app.put('/api/hero-slides', async (req: Request, res: Response) => {
  try {
    ...
    // Convert camelCase request to DB format
    ...
    if (updates.buttonText !== undefined && (!updates.buttonText || !updates.buttonText.trim())) {
      return res.status(400).json({ 
    ...
    if (updates.buttonLink !== undefined && (!updates.buttonLink || !updates.buttonLink.trim())) {
      return res.status(400).json({ 
    ...
    if (updates.buttonText) updates.buttonText = updates.buttonText.trim();
    if (updates.buttonLink) updates.buttonLink = updates.buttonLink.trim();
